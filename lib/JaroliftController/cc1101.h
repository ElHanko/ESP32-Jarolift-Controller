 ...
 * Carrier frequencies
 */
enum CFREQ { CFREQ_868 = 0, CFREQ_915, CFREQ_433, CFREQ_918, CFREQ_LAST };
...
#define CC1101_SFSTXON                                                                                                                               \
  0x31                    // Enable and calibrate frequency synthesizer (if MCSM0.FS_AUTOCAL=1). If in RX (with CCA):
...
public:
   ...
   */
  bool init(uint8_t freq = CFREQ_868);
  ...
  void setGPIO(int8_t sck, int8_t miso, int8_t mosi, int8_t ss, int8_t gdo0);
